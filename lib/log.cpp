...
bool log_init(LogLevel l, const char* p_modulename, const char* p_logdir)
{
	...
	snprintf(_location_str, _LOG_PATH_LEN, "%s/log-%s", p_logdir, p_modulename);	
...
int Log_Writer::premakestr(char* m_buffer, LogLevel l)
{
	...
	struct tm vtm; 
    localtime_r(&now, &vtm);
    return snprintf(m_buffer, _LOG_BUFFSIZE, "%02d-%02d %02d:%02d:%02d [%s]",
            vtm.tm_mon + 1, vtm.tm_mday, vtm.tm_hour, vtm.tm_min, vtm.tm_sec, logLevelToString(l));
}
